...
async def disability_prediction_page(
    ...
    try:
        ...
        # CDN 문제로 인해 간단한 페이지로 대체
        with open("app/templates/pages/analysis/disability_simple.html", "r", encoding="utf-8") as f:
            html_content = f.read()

        # CSRF 토큰 교체
        html_content = html_content.replace('value="{{ csrf_token if csrf_token else \'dummy-token\' }}"', f'value="{csrf_token}"')
...
async def disability_results_page(
    ...
    try:
        # CDN 문제로 인해 간단한 페이지로 대체
        with open("app/templates/pages/analysis/disability_results_simple.html", "r", encoding="utf-8") as f:
            html_content = f.read()
